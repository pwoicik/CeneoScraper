...
class Review:
    def __init__(self, review_container: element.Tag):
        self.id = int(review_container["data-entry-id"])
        ...
        self.score = None if len(score) == 0 else float(score[0].text.split("/")[0].replace(",", "."))
        ...
        self.issue_date = dates[0]["datetime"]
        self.purchase_date = dates[0]["datetime"]

        self.yes_votes = int(review_container.select("button.vote-yes")[0]["data-total-vote"])
        self.no_votes = int(review_container.select("button.vote-no")[0]["data-total-vote"])

        self.content = review_container.select("p.product-review-body")[0].text.strip()
        self.pros = "".join([pro.text.strip() for pro in review_container.select("div.pros-cell > ul > li")])
        self.cons = "".join([con.text.strip() for con in review_container.select("div.cons-cell > ul > li")])
